...
class Field:
    ...
    def draw_grid(self, width, height):
        ...
        for i in range(
            self.y_offset, self.y_offset + grid_height + 1, self.size_factor
        ):
            self.canvas.create_line(
                self.x_offset,
                i,
    ...
    def draw_robot(self, x, y, theta, preview):
        ...
        for corner in corners:
            rotated_x = corner[0] * math.cos(theta_rad) - corner[1] * math.sin(
                theta_rad
            )
            rotated_y = corner[0] * math.sin(theta_rad) + corner[1] * math.cos(
                theta_rad
            )
            rotated_corners.append(
                (rotated_x + x + self.x_offset, rotated_y + y + self.y_offset)
            )

        if preview is False:
            self.clear_robot()
            self.robot_image = self.canvas.create_polygon(
                rotated_corners, fill="blue", outline="blue", tags="robot"
            )
            if robot_x < robot_y:
                self.robot_center = self.canvas.create_oval(
                    x - robot_x / 6 + self.x_offset,
                    y - robot_x / 6 + self.y_offset,
                    x + robot_x / 6 + self.x_offset,
                    y + robot_x / 6 + self.y_offset,
                    fill="red",
                    tags="robot_center",
                ...
                self.robot_center = self.canvas.create_oval(
                    x - robot_y / 6 + self.x_offset,
                    y - robot_y / 6 + self.y_offset,
                    x + robot_y / 6 + self.x_offset,
                    y + robot_y / 6 + self.y_offset,
                    fill="red",
                    tags="robot_center",
            ...
            robot_preview = self.canvas.create_polygon(
                rotated_corners, fill="", outline="blue", dash=(5, 2), tags="robot"
            )
    ...
    def draw_path(self, path):
        ...
        scaled_path = [
            (
                point.get_x() * self.size_factor + self.x_offset,
        ...
        for i in range(0, len(self.path) - 1):
            ...
            for j in range(0, (int)(50 * time + 1)):
                if self.preview_number != preview_number:
                    return
                self.draw_robot(
                    self.path[i].get_x() + x_dif * j / time / 50,
                    self.path[i].get_y() + y_dif * j / time / 50,
                    self.path[i].get_angle() + angle * j / time / 50,
                    False,
                )
                start_time = tm.time()
                self.canvas.update()
